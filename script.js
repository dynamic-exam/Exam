...
function showReport(){
    ...
    reportFilterSticky();
    document.querySelectorAll('.questions .report-card').forEach(q => {
        observer.observe(q);
    });
}
...
function setExam(){
            ...
            op += `<label>
                        <div class="option">
                            <div class="p1" onclick="clickedAns(${i}, ${a});">
                                <div role="radio" class="radio">${opLabel[a]}</div>
                            </div>
                            <div class="p2">${questions[i].options[a]}</div>
                        </div>
                    </label>
    ...
    document.querySelectorAll('.questions .exam-card').forEach(q => {
        observer.observe(q);
    });
}
  ...
  entries.forEach(async entry => {
    if (entry.isIntersecting) {
      if (!entry.target.dataset.rendered) {
        await MathJax.typesetPromise([entry.target]);
