window.onerror = function(message, source, lineno, colno, error) {
    Alert(
        `${message}\n` +
        `File: ${source}\n`  +
        `Line: ${lineno} Col: ${colno}`, error
    );
};

window.addEventListener("load", () => {
    hideLoader();
});

document.title = title+' | Dynamic Exam System';

const examId = get('examId');
setUpWebApp();

async function setUpWebApp(){
    showLoader();
    if(localStorage.getItem('exam'+examId) == 'submitted'){
        try {
            showReport();
        }
        finally {
            hideLoader();
        }
    }
    else{
        try {
            showFrontPage();
        }
        finally {
            hideLoader();
        }
    }
}


function showFrontPage(){
    document.querySelector('.info-page').style.display = 'flex';
    document.querySelector('.info-page .exam-title').innerText = title;
    document.querySelector('.info-page .exam-info #info-total .data').innerText = String(questions.length).padStart(2, '0');
    document.querySelector('.info-page .exam-info #info-time .data').innerText = String(time).padStart(2, '0');
    document.querySelector('.info-page .instructions #info-total').innerText = questions.length;
    document.querySelector('.info-page .instructions #info-time').innerText = time;
    document.querySelector('.info-page .instructions #info-negMark').innerText = negMark;
}

function showReport(){
    let ansScript = JSON.parse(localStorage.getItem("examScript"+examId)) || new Array(questions.length).fill('blank');

    var corr = 0;
    var incr = 0;
    var blank = 0;

    const qContainer = document.querySelector('.container .questions');
    qContainer.innerHTML = '';
    document.querySelector('.container .head .title').innerText = title;
    document.querySelector('.container .head .time').innerHTML ='<i class="fa fa-clock-o" aria-hidden="true"></i> ' + String(time).padStart(2, '0') + ' Minutes';

    for(var i=0; i<questions.length; i++){
        var card = '';
        var op = '';
        var explanation = '';
        for(var a=0; a<questions[i].options.length; a++){
            op += `<label>
                        <div class="option">
                            <div class="p1">
                                <div role="radio" title="${opLabel[a]}" class="radio">${opLabel[a]}</div>
                            </div>
                            <div class="p2">${questions[i].options[a]}</div>
                        </div>
                    </label>
                    <div class="option-border"></div>`;
        }
        if(questions[i].explanation){
            explanation = `<div class="explanation">
                            <div class="exLabel"><i class="fa fa-lightbulb-o" aria-hidden="true"></i> Explanation:</div>
                            <div class="exText">
                            ${questions[i].explanation}
                            </div>
                        </div>`;
        }
        card = `<div class="report-card" id="card${i}">
                <div class="top-part">
                    <div class="qLabel">Question ${i+1}</div>
                    <div class="qStatus"></div>
                </div>
                <div class="question">${questions[i].question}</div>
                <div class="options">
                    ${op}
                </div>
                ${explanation}
            </div>`;
        
        qContainer.innerHTML += card;

        if(ansScript[i] == questions[i].corrAns){
            corr ++;
            document.querySelector('.report-card#card'+i+' .top-part .qStatus').innerHTML = `<span class="corr">Correct</span>`;
            document.querySelectorAll('.report-card#card'+i+' .options .option')[questions[i].corrAns].className = 'option correct';
            document.querySelectorAll('.report-card#card'+i+' .options .option')[questions[i].corrAns].querySelector('.p2').innerHTML += '<i id="tick" class="fa fa-check" aria-hidden="true"></i>';
        }
        else if(ansScript[i] == 'blank'){
            blank ++;
            document.querySelector('.report-card#card'+i+' .top-part .qStatus').innerHTML = `<span class="blank">Blank</span>`;
            document.querySelectorAll('.report-card#card'+i+' .options .option')[questions[i].corrAns].className = 'option correct-w';
            document.querySelectorAll('.report-card#card'+i+' .options .option')[questions[i].corrAns].querySelector('.p2').innerHTML += '<i id="tick" class="fa fa-check" aria-hidden="true"></i>';
        }
        else{
            incr ++;
            document.querySelector('.report-card#card'+i+' .top-part .qStatus').innerHTML = `<span class="incr">Incorrect</span>`;
            document.querySelectorAll('.report-card#card'+i+' .options .option')[ansScript[i]].className = 'option incorrect';
            document.querySelector('.report-card#card'+i+' .options .option.incorrect .p2').innerHTML += '<i id="cross" class="fa fa-times" aria-hidden="true"></i>';
            document.querySelectorAll('.report-card#card'+i+' .options .option')[questions[i].corrAns].className = 'option correct-w';
            document.querySelectorAll('.report-card#card'+i+' .options .option')[questions[i].corrAns].querySelector('.p2').innerHTML += '<i id="tick" class="fa fa-check" aria-hidden="true"></i>';
        }
    }

    var obtained = corr*mark - incr*negMark;
    var total = questions.length*mark;
    var accuracy = (corr+incr) == 0? 0: Math.round((corr / (corr+incr))*100);

    document.querySelector('.container .report .marks .data').innerText = obtained+'/'+total;
    document.querySelector('.container .report #correct .data').innerText = String(corr).padStart(2, '0');
    document.querySelector('.container .report #incorrect .data').innerText = String(incr).padStart(2, 0);
    document.querySelector('.container .report #skipped .data').innerText = String(blank).padStart(2, 0);
    document.querySelector('.container .report #accuracy .data').innerText = String(accuracy).padStart(2, '0') + '%';
    document.querySelector('.container .report').style.display = 'block';
    document.querySelector('.container .report-card-filter').style.display = 'flex';
    reportFilterSticky();
    if (window.MathJax) {
        MathJax.typesetPromise();
    }
}


function startExam(){
    var name = document.querySelector('.info-page .info-box .info-form #name');
    var college = document.querySelector('.info-page .info-box .info-form #college');
    if(name.value == ''){
        name.focus();
    }
    else if(college.value == ''){
        college.focus();
    }
    else{
        document.querySelector('.info-page .form-btns #start-exam-btn').innerHTML='Wait...';
        document.querySelector('.info-page .form-btns #start-exam-btn').style.pointerEvents='none';
        showLoader();
        setExam();
        document.querySelector('.info-page').style.display = 'none';
        setTimeout(()=>{
            hideLoader();
        }, 2000);
    }
}


function setExam(){
    document.querySelector('.container .questions').innerHTML = '';
    document.querySelector('.container .head .title').innerText = title;
    document.querySelector('.container .head .time').innerHTML ='<i class="fa fa-clock-o" aria-hidden="true"></i> ' + time+' Minutes';

    for(var i=0; i<questions.length; i++){
        var card = '';
        var op = '';
        for(var a=0; a<questions[i].options.length; a++){
            op += `<label>
                        <div class="option">
                            <div class="p1" onclick="clickedAns(${i}, ${a});">
                                <div role="radio" title="${opLabel[a]}" class="radio"><div class="radio-inner">${opLabel[a]}</div></div>
                            </div>
                            <div class="p2">${questions[i].options[a]}</div>
                        </div>
                    </label>
                    <div class="option-border"></div>`;
        }
        card = `<div class="exam-card" id="card${i}">
                <div class="top-part">
                    <div class="qLabel">Question ${i+1}</div>
                </div>
                <div class="question">${questions[i].question}</div>
                <div class="options">
                    ${op}
                </div>
            </div>`;
        
        document.querySelector('.container .questions').innerHTML+= card;
    }
    startTimer(time * 60);
    document.querySelector('.fixed-bar').style.display = 'flex';
    document.querySelector('.container .foot #submit-btn').style.display = 'flex';
    if (window.MathJax) {
        MathJax.typesetPromise();
    }
}
var ansScript1 = new Array(questions.length).fill('blank');



function clickedAns(q,o){
    ansScript1[q] = o;
    document.querySelector('.container .questions .exam-card#card'+q).className= 'exam-card answered';
    document.querySelectorAll('.container .questions .exam-card#card'+q+' .options .option')[o].className = 'option answered';
}

function startTimer(duration) {
    let timeLeft = duration;
    const timerSpan = document.querySelector('.fixed-bar .time');

    const interval = setInterval(() => {
        let minutes = Math.floor(timeLeft / 60);
        let seconds = timeLeft % 60;
        minutes = minutes < 10 ? '0' + minutes : minutes;
        seconds = seconds < 10 ? '0' + seconds : seconds;

        timerSpan.innerHTML ='<i class="fa fa-clock-o" aria-hidden="true"></i> <span>'+ minutes + ' : ' + seconds + '</span>';

        if (timeLeft <= 0) {
            clearInterval(interval);
            timerSpan.textContent = "Time's up!";
            submit();
        }
        timeLeft--;
    }, 1000);
}



function submit(){
    showLoader();
    var corr = 0;
    var incr = 0;
    var blank = 0;
    var ansScript2 = '';
    for(var i=0; i<questions.length; i++){
        if(ansScript1[i] == questions[i].corrAns){
            corr ++;
            ansScript2+= ' '+opLabel[ansScript1[i]]+',';
        }
        else if(ansScript1[i] == 'blank'){
            blank ++;
            ansScript2+=' blank,';
        }
        else{
            incr ++;
            ansScript2+= ' '+opLabel[ansScript1[i]]+',';
        }
    }
    var obtained = corr*mark - incr*negMark;
    var total = questions.length*mark;
    var accuracy = (corr+incr) == 0? 0: Math.round((corr / (corr+incr))*100);

    const url = sheet;

    const formData = new FormData();
    formData.append("name", document.querySelector('.info-page #name').value);
    formData.append("college", document.querySelector('.info-page #college').value);
    formData.append("examName", title);
    formData.append("total", total);
    formData.append("obtained", obtained);
    formData.append("correct", corr);
    formData.append("incorrect", incr);
    formData.append("skipped", blank);
    formData.append("accuracy", accuracy+'%');
    formData.append("ansScript", '[' + ansScript2 + ']');

    fetch(url, {
        method: "POST",
        body: formData,
        mode: "no-cors"
    })
    .then(() => {
        localStorage.setItem('examScript'+examId, JSON.stringify(ansScript1));
        localStorage.setItem('exam'+examId, 'submitted');
        window.location.href = window.location.href;
    })
    .catch(err => {
        Alert('Please check your internet connection and try again.', 'Could not submit!');
        hideLoader();
    });
}


//confirm msg box
function showConfirmation(){
    document.querySelector('.confirm-box-filter').style.opacity=1;
    document.querySelector('.confirm-box-filter').style.pointerEvents='all';
    document.querySelector('.confirm-box-filter .confirm-box').style.transform='scale(1)';
}
function hideConfirmation(){
    document.querySelector('.confirm-box-filter').style.opacity=0;
    document.querySelector('.confirm-box-filter').style.pointerEvents='none';
    document.querySelector('.confirm-box-filter .confirm-box').style.transform='scale(0.9)';
}
document.querySelector('.confirm-box-filter').addEventListener('click', (e)=>{
    if(e.target.className == 'confirm-box-filter'){
        hideConfirmation();
    }
});


//alert msg box
function Alert(msg, title){
    document.querySelector('.alert-box-filter .alert-box .msg-title').innerText = title || 'Alert';
    document.querySelector('.alert-box-filter .alert-box .msg').innerText = msg || 'No msg found';
    document.querySelector('.alert-box-filter').style.opacity=1;
    document.querySelector('.alert-box-filter').style.pointerEvents='all';
    document.querySelector('.alert-box-filter .alert-box').style.transform='scale(1)';
}
function hideAlert(){
    document.querySelector('.alert-box-filter').style.opacity=0;
    document.querySelector('.alert-box-filter').style.pointerEvents='none';
    document.querySelector('.alert-box-filter .alert-box').style.transform='scale(0.9)';
}
document.querySelector('.alert-box-filter').addEventListener('click', (e)=>{
    if(e.target.className == 'alert-box-filter'){
        hideAlert();
    }
});



//loader screen
function showLoader(){
    document.querySelector('.loader-screen').style.opacity=1;
    document.querySelector('.loader-screen').style.pointerEvents='all';
}
function hideLoader(){
    document.querySelector('.loader-screen').style.opacity=0;
    document.querySelector('.loader-screen').style.pointerEvents='none';
}


//question filter box
function openFilterBox(){
    document.querySelector('.filter-box-filter').style.pointerEvents = 'all';
    document.querySelector('.filter-box').style.pointerEvents = 'all';
    document.querySelector('.filter-box').style.opacity = 1;
    document.querySelector('.filter-box').style.transform = 'scale(1)';
}
function closeFilterBox(){
    document.querySelector('.filter-box-filter').style.pointerEvents = 'none';
    document.querySelector('.filter-box').style.pointerEvents = 'none';
    document.querySelector('.filter-box').style.opacity = 0;
    document.querySelector('.filter-box').style.transform = 'scale(0.95)';
}
function filter(filter){
    document.querySelectorAll('.filter-box .options .option').forEach(option =>{
        option.className = 'option';
    });
    if(filter == 'all'){
        document.querySelectorAll('.container .questions .exam-card').forEach(card =>{
            card.style.display = 'block';
        });
        document.querySelector('.fixed-bar .filter-btn .label').innerText = 'All';
        document.querySelector('.filter-box .options .option#all').className = 'option active';
    }
    else if(filter == 'answered'){
        for(var i=0; i<questions.length; i++){
            if(ansScript1[i]!='blank'){
                document.querySelectorAll('.container .questions .exam-card')[i].style.display = 'block';
            }
            else{
                document.querySelectorAll('.container .questions .exam-card')[i].style.display = 'none';
            }
        }
        document.querySelector('.fixed-bar .filter-btn .label').innerText = 'Answered';
        document.querySelector('.filter-box .options .option#answered').className = 'option active';
    }
    else if(filter == 'unanswered'){
        for(var i=0; i<questions.length; i++){
            if(ansScript1[i]=='blank'){
                document.querySelectorAll('.container .questions .exam-card')[i].style.display = 'block';
            }
            else{
                document.querySelectorAll('.container .questions .exam-card')[i].style.display = 'none';
            }
        }
        document.querySelector('.fixed-bar .filter-btn .label').innerText = 'Unanswered';
        document.querySelector('.filter-box .options .option#unanswered').className = 'option active';
    }
    closeFilterBox();
}



//report filter grid
function filterReport(filter){
    let ansScript = JSON.parse(localStorage.getItem("examScript"+examId)) || new Array(questions.length).fill('blank');
     document.querySelectorAll('.container .report-card-filter .option').forEach(option =>{
        option.className = 'option';
    });
    if(filter == 'all'){
        document.querySelectorAll('.container .questions .report-card').forEach(card =>{
            card.style.display = 'block';
        });
        document.querySelector('.container .report-card-filter .option#all').className = 'option active';
    }
    else if(filter == 'correct'){
        for(var i=0; i<questions.length; i++){
            if(ansScript[i] == questions[i].corrAns){
                document.querySelectorAll('.container .questions .report-card')[i].style.display = 'block';
            }
            else{
                document.querySelectorAll('.container .questions .report-card')[i].style.display = 'none';
            }
        }
        document.querySelector('.container .report-card-filter .option#correct').className = 'option active';
    }
    else if(filter == 'incorrect'){
        for(var i=0; i<questions.length; i++){
            if(ansScript[i] != questions[i].corrAns && ansScript[i]!='blank'){
                document.querySelectorAll('.container .questions .report-card')[i].style.display = 'block';
            }
            else{
                document.querySelectorAll('.container .questions .report-card')[i].style.display = 'none';
            }
        }
        document.querySelector('.container .report-card-filter .option#incorrect').className = 'option active';
    }
    else if(filter == 'skipped'){
        for(var i=0; i<questions.length; i++){
            if(ansScript[i] == 'blank'){
                document.querySelectorAll('.container .questions .report-card')[i].style.display = 'block';
            }
            else{
                document.querySelectorAll('.container .questions .report-card')[i].style.display = 'none';
            }
        }
        document.querySelector('.container .report-card-filter .option#skipped').className = 'option active';
    }
}

function reportFilterSticky(){
    var reportFilterH = document.querySelector('.container .report-card-filter').clientHeight;
    window.addEventListener('scroll', () => {
        var headH = document.querySelector('.container .head').clientHeight;
        var reportH = document.querySelector('.container .report').clientHeight;
        var triggerH = headH + reportH + 20;
        if (window.scrollY >= triggerH) {
            document.querySelector('.report-card-filter').id="sticky";
        }
        else {
            document.querySelector('.report-card-filter').id="";
        }
    });
}